...
def _check_accuracy(output, target):
...
def train(model, batched_train_data, batched_train_label, optimizer, criterion, scheduler, device, num_classes):
    model.train()

    acc_training_batches = []

    # Record total loss
    total_loss = 0.

    # Get the progress bar for later modification
    progress_bar = tqdm_notebook(batched_train_data, ascii=True)

    # Mini-batch training
    for batch_idx, a_train_data_batch in enumerate(progress_bar):
        #source = data[0].transpose(1, 0).to(device)
        #target = data[1].transpose(1, 0).to(device)
        source = a_train_data_batch
        target = batched_train_label[batch_idx]
        ...
        # no need to do this, as nn.CrossEntropyLoss can take either/or.
        ...
        #print("train: yhat: ", translation, ", y: ", target)
        #print("yhat.shape: ", translation.shape, ", y.shape: ", target.shape)
        ...
        acc_train_batch = _check_accuracy(translation, target)
        acc_training_batches.append(acc_train_batch)

        progress_bar.set_description_str(
            "Batch: %d, Loss: %.4f" % ((batch_idx + 1), loss.item()))

    #print(acc_training_batches)
    return total_loss, total_loss / len(batched_train_data), torch.mean(torch.stack(acc_training_batches))


def evaluate(model, batched_valid_data, batched_valid_label, criterion, device, num_classes):
    ...
    with torch.no_grad():
        ...
        progress_bar = tqdm_notebook(batched_valid_data, ascii=True)
        for batch_idx, a_valid_data_batch in enumerate(progress_bar):
            #source = data[0].transpose(1, 0).to(device)
            #target = data[1].transpose(1, 0).to(device)
            source = a_valid_data_batch
            target = batched_valid_label[batch_idx]
            ...
            acc_valid_batch = _check_accuracy(translation, target)
            acc_valid_batches.append(acc_valid_batch)

            progress_bar.set_description_str(
                "Batch: %d, Loss: %.4f" % ((batch_idx + 1), loss.item()))

    avg_loss = total_loss / len(batched_valid_data)

    #print(acc_valid_batches)
    return total_loss, avg_loss, torch.mean(torch.stack(acc_valid_batches))


def plot_curves(train_loss_history, valid_loss_history, train_acc_history, valid_acc_history, filename):
    '''
    ...
    axs[0].plot(epochs, train_loss_history, label='train')
    axs[0].plot(epochs, valid_loss_history, label='valid')
    axs[0].set_xlabel('Epochs')
    axs[0].set_ylabel('Loss')
    axs[0].legend()
    axs[0].set_title('Loss Curve - ' + filename)

    epochs = range(len(train_acc_history))
    axs[1].plot(epochs, train_acc_history, label='train')
    axs[1].plot(epochs, valid_acc_history, label='valid')
    axs[1].set_xlabel('Epochs')
    axs[1].set_ylabel('Accuracy')
    axs[1].legend()
    axs[1].set_title('Accuracy Curve - ' + filename)
